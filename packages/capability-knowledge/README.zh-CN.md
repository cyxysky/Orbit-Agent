...
`ingest` 后保存返回的文档 ID。`search` 使用 `query`，`get` 使用 `documentId`，`list` 支持分页，`delete` 使用准确 ID。检索结果包含文档和分块来源，生成回答时应保留来源信息。Node 存储使用 SQLite FTS5 和中文二元分词，不是向量嵌入服务。

`content` 应是已提取的文本；PDF/Office 上传文件先通过 File 或自有导入流程提取。使用固定且由宿主决定的目录保持持久化，不同租户使用不同目录。备份时保持数据库与 WAL 一致，删除存储前先 dispose。可通过 `createKnowledgeCapability` 注入 `KnowledgeStore` 实现向量或混合检索。
