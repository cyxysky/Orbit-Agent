...
npm install @webpilot/capability-knowledge @webpilot/capability-sdk @webpilot/capability-host
npm install -D typescript tsx @types/node
```

`ingest` 後に返された文書 ID を保存します。`search` には `query`、`get` には `documentId`、`delete` には正確な ID を渡し、`list` はページ単位で取得します。検索結果の文書・チャンクの出典を回答にも保持します。Node ストアは中国語 bigram 分割を備えた SQLite FTS5 であり、埋め込みサービスではありません。

`content` は抽出済みテキストです。PDF/Office のアップロードは File または自分の取り込み処理で先に抽出します。永続的な検索にはホストが決めた固定ディレクトリを使い、テナントごとに分離します。バックアップは DB と WAL の整合性を保ち、削除前に dispose します。`createKnowledgeCapability` に独自の `KnowledgeStore` を渡せばベクトル・ハイブリッド検索も実装できます。
...
- `@webpilot/capability-knowledge`
- `@webpilot/capability-knowledge/node`
- `@webpilot/capability-knowledge/mcp`
- `@webpilot/capability-knowledge/runtime-skill`
- `@webpilot/capability-knowledge/settings`
