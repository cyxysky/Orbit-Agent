...
已有隔离 HTTP runner 时，从 `/remote` 使用 `createHttpCodeSandboxCapability({ url, token })`，runner 由使用方独立部署。仅设置 `AGENT_CODE_SANDBOX_BACKEND` 不会改变显式选择的工厂函数。本地工厂无法实现 `networkMode: "none"`，CPU、内存和网络隔离需要执行后端提供。
