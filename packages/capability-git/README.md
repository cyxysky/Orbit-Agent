...
Inspect and explicitly modify one host-selected Git repository.
...
Install Git >=2.30 on PATH and set `AGENT_REPOSITORY` to an existing repository. `status`, `diff`, `log`, `show` and `branches` inspect it. `applyPatch` needs a unified `patch`; `commit` needs a message and explicit repository-relative `paths`.

Writes require `AGENT_GIT_ALLOW_WRITES=true` and host authorization. The declared tool permissions are coarse and include repository writes even when the operation is a read; they do not decide action-level approval. Use the integration `beforeInvoke` hook for that decision. Git credentials, repository selection and any remote/network access belong to the host. This package does not create a repository or supply an Agent loop.
