...
ホストが選んだ Git リポジトリを調査し、明示的に変更します。
...
PATH 上に Git >=2.30 が必要です。`AGENT_REPOSITORY` は既存リポジトリを指します。`status`、`diff`、`log`、`show`、`branches` で調査し、`applyPatch` には unified diff の `patch`、`commit` にはメッセージと明示的なリポジトリ相対 `paths` を渡します。

書き込みには `AGENT_GIT_ALLOW_WRITES=true` とホストの承認が必要です。宣言された権限は粗い粒度で、読み取りでも書き込み権限を含むため、操作ごとの承認を代替しません。接続コードの `beforeInvoke` で判定できます。Git 認証、対象リポジトリ、リモートアクセスはホストが管理します。本パッケージはリポジトリを作成せず、Agent Loop も提供しません。
