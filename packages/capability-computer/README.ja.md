...
ローカルまたはリモートのドライバーで対話型デスクトップを観測・操作します。
...
内蔵ドライバーにはロックされていない Windows の対話型ユーザーセッションが必要で、Session 0 では使えません。`AGENT_COMPUTER_ENABLED` を有効にすると遅延起動します。他の OS には `AGENT_COMPUTER_ENDPOINT` または独自 `createDriver` が必要です。endpoint はドライバーの HTTP API で、RDP/VNC アドレスではありません。`HEAD` はヘルスチェック、`POST` は操作 JSON を受け取り、任意の `AGENT_COMPUTER_AUTHORIZATION` を Authorization ヘッダーとして送ります。

先に observe し、現在の `elementId` または返された画像サイズ内の正確なピクセル座標でクリックします。操作後に ID は無効になります。対応操作は `observe`、`launch`、`click`、`type`、`key`、`scroll`、`wait` です。observe は既に画像を取得します。汎用のテキスト例は結果構造を保持しますが画像をモデルに表示しないため、信頼されたストレージから画像を解決する変換処理を追加します。MCP では `resolveImage` を使い、アダプターは任意の成果物 URL を自動取得しません。
