...
保存返回的工作流 ID。`read` 读取记录；`updateStep` 接收 `workflowId`、`stepId`、`stepStatus` 以及可选的已核实 `output`；`cancel` 结束工作流。依赖步骤完成后才能开始或完成后续步骤。重复提交相同检查点是幂等的，但 completed/failed/cancelled 为终态；重试失败任务应新建工作流。

本包保存记录，不会调度工具、运行 Agent Loop 或自动重放中断的任务。宿主在重启后读取状态并决定执行什么。SQLite 目录应固定并归属对应工作区，使用结束后释放运行实例。
