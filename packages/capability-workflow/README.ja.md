...
依存関係を持つワークフローと確認済みチェックポイントを永続化します。
...
返されたワークフロー ID を保存します。`read` で記録を読み、`updateStep` に `workflowId`、`stepId`、`stepStatus` と確認済みの `output` を渡し、`cancel` で終了します。依存するステップは前提ステップの完了後に開始・完了できます。同一チェックポイントの再送は冪等ですが completed/failed/cancelled は終端状態です。失敗した作業の再試行は新しいワークフローとして作成します。

本パッケージは記録を保存するもので、ツールのスケジュール、Agent Loop、中断作業の自動再実行は行いません。ホストが再起動後に状態を読み、実行内容を決めます。SQLite ディレクトリをワークスペースごとに固定し、終了時に実行インスタンスを解放します。
