...
Success is reported by outer ok; successful cells omit executionState. Failed cells preserve executionState. Downloads, images and imageErrors are returned only when non-empty, error only when present, and aborted only when true. Code results may include executionState and downloads. executionState distinguishes attempted actions from completed Playwright calls and marks timeout/abort/crash outcomes unknown after execution starts. Refresh live state before retrying; never replay a submission solely because the cell timed out. kernelReset also reports timeout/aborted/crashed and means previous JavaScript bindings are gone. downloads contains persisted artifact IDs when the host supplies a receiver; pass those artifact IDs to file readContent/convert instead of fetching the export URL again.

The outer browser action=code result is \`{ ok, summary, data, failureCategory?, referenceImagePaths? }\`. \`summary\` is a short description; \`data\` is the single structured payload containing \`{ result, finalPage, error?, aborted?, executionState?, downloads?, kernelReset?, verification?, domChanges?, images?, imageErrors? }\`. Read \`data\` directly without JSON parsing. \`needChange\` defaults to false: omit it for ordinary calls to skip reading and returning \`domChanges\`. Set \`needChange: true\` on the same cell when its incremental DOM changes or diagnostics are needed. Changes from earlier cells are not retained for later retrieval; use an explicit snapshot or targeted read to inspect current state. \`domChanges\` is only the incremental journal caused by that cell, not a full snapshot. Failed results preserve the complete error and failure classification without generated recovery prose.
