...
For PPTX, author editable native text, shapes and connectors through the planned JavaScript or UNO engine. The source references' SVG coordinates and CSS are design examples, not Office API arguments. JavaScript uses PptxGenJS shape/text APIs and native chart/table APIs; query jsApi for the current contract. UNO uses its documented slide text/shape/connect/chart/table methods; query unoApi for exact signatures. Do not invent an API or switch an existing draft's engine to follow a visual example.

Map a logical drawing canvas into the allocated figure rectangle: scale = min(availableWidth / canvasWidth, availableHeight / canvasHeight), then center the result. Keep API-specific units separate: PptxGenJS positions are inches and type sizes are points; UNO APIs use their documented units. Recheck readable type sizes at the final destination instead of blindly scaling web pixels. A figure designed for a blog column may need larger labels and fewer overview nodes on a projected slide.
