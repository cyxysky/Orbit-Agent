...
示例打开已有的文件型 SQLite 数据库，不创建表。需要安装 TypeORM 与 better-sqlite3；PostgreSQL 改用 pg 驱动和相应 DataSource。DataSource 由宿主持有，并在能力释放后 destroy。

按 `sources → schema(sourceId) → query(sourceId, statement, parameters, maxRows)` 调用，参数占位符遵循对应数据库方言，保留 `columns`、`rows`、`rowCount`、`truncated`、`elapsedMs`。TypeORM 实现对 SELECT/CTE 查询施加限制，支持 PostgreSQL 与文件型 SQLite，不支持内存 SQLite。PostgreSQL 使用只读事务和取消机制，SQLite 使用只读 worker。默认查询超时 15 秒，可设置数据源适配器的 `timeoutMs`。其他数据库实现 `AgentDataSource`；声明权限不会把只读数据源变成可写。
