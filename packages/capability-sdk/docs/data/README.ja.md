...
注入したドライバーで SQL データソースを検出し、制限付きクエリを実行します。
...
例は既存のファイル型 SQLite を開くだけで、テーブルを作成しません。TypeORM と better-sqlite3 が必要です。PostgreSQL では pg と対応する DataSource を使います。DataSource はホストが所有し、Capability 解放後に destroy します。

`sources → schema(sourceId) → query(sourceId, statement, parameters, maxRows)` の順に呼び出します。プレースホルダーは SQL 方言に合わせ、`columns`、`rows`、`rowCount`、`truncated`、`elapsedMs` を保持します。TypeORM 実装は SELECT/CTE に制限を適用し、PostgreSQL とファイル型 SQLite をサポートしますが、メモリ内 SQLite は対象外です。PostgreSQL は読み取り専用トランザクションと中断、SQLite は読み取り専用 worker を使います。既定のクエリタイムアウトは 15 秒で、データソースの `timeoutMs` で設定できます。他の DB は `AgentDataSource` を実装します。権限の宣言だけでは読み取り専用ソースは書き込み可能になりません。
