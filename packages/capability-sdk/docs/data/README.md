# @cjfclonedeep/capability-sdk/data
...
Discover SQL sources and execute bounded queries through injected drivers.
...
npm install @cjfclonedeep/capability-sdk  typeorm better-sqlite3
npm install -D typescript tsx @types/node
```

The example opens an existing file-backed SQLite database; no tables are created. Install TypeORM and better-sqlite3. PostgreSQL instead needs `pg` and a PostgreSQL DataSource. The host owns and destroys the DataSource after capability disposal.

Call `sources → schema(sourceId) → query(sourceId, statement, parameters, maxRows)`. Use parameter placeholders for the selected dialect and preserve `columns`, `rows`, `rowCount`, `truncated`, `elapsedMs`. The TypeORM implementation bounds SELECT/CTE queries and supports PostgreSQL or file-backed SQLite, not in-memory SQLite. PostgreSQL uses read-only transactions and cancellation; SQLite uses a read-only worker. Default query timeout is 15 seconds; configure the data-source adapter's `timeoutMs`. Other engines implement the exported `AgentDataSource` contract. Declared permissions do not turn a read-only source into a writable one.
...
import { DataSource } from 'typeorm';
 import { createTypeOrmAgentDataSource, createTypeOrmDataCapability } from '@cjfclonedeep/capability-sdk/data/typeorm';
 const database = process.env.AGENT_SQLITE_PATH;
 if (!database) throw new Error('Set AGENT_SQLITE_PATH to an existing SQLite file');
 const dataSource = new DataSource({ type: 'better-sqlite3', database, entities: [], synchronize: false });
 await dataSource.initialize();
 const provider = createTypeOrmDataCapability({ sources: [
   createTypeOrmAgentDataSource({ id: 'application', source: dataSource, readOnly: true }),
 ] });
...
export const exampleCall = {
  ...
  "input": {
    "action": "sources",
    "reason": "Discover configured databases"
  }
};
export async function cleanup() { await dataSource.destroy(); }
```
...
| --- | --- | --- |
| `AGENT_DATA_MAX_ROWS` | `500` | `runtime` |
| `AGENT_DATA_ALLOW_WRITES` | `false` | `runtime` |
...
- `@cjfclonedeep/capability-sdk/data`
- `@cjfclonedeep/capability-sdk/data/typeorm`
- `@cjfclonedeep/capability-sdk/data/mcp`
- `@cjfclonedeep/capability-sdk/data/runtime-skill`
- `@cjfclonedeep/capability-sdk/data/settings`
...
} from '@cjfclonedeep/capability-sdk/data/typeorm';
