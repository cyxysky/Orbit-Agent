...
URL 指向已有外部 MCP 服务；本包充当该服务的客户端，而 adapter-mcp 用于发布你自己的工具。凭证由宿主配置。`list` 返回连接 ID，`describe` 接收 `connectorId` 并返回准确操作 ID 和参数结构，`call` 需要两个 ID 及 `arguments`。先发现操作，再调用。

使用工厂为每个运行实例创建连接，避免释放一次运行时关闭其他运行的客户端。操作列表缓存 60 秒，MCP 分页和 SSE 由内部处理。MCP 的 `isError` 会保留为失败，过长结果带有截断信息。OpenAPI 适配保留基础 URL 的路径前缀。自定义 `AgentConnector` 实现 `listOperations`、`call` 和可选的 health/dispose；外部写入的操作级授权由宿主负责。
...
阅读本包随附、可独立使用的 [MCP 完整教程](CONNECTORS-MCP.zh-CN.md)。包含依赖安装、stdio 进程、有状态 HTTP 监听服务、客户端发现与调用、模型驱动的客户端 Agent、取消、鉴权边界和关闭流程。复用上面的 provider.ts 与 policy.ts。远程客户端只需 MCP URL 及客户端依赖，不导入本能力包。
