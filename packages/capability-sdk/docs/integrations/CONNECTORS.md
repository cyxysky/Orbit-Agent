# @cjfclonedeep/capability-sdk/integrations/connectors

[English](CONNECTORS.md) | [简体中文](CONNECTORS.zh-CN.md) | [日本語](CONNECTORS.ja.md)
...
Discover and call external MCP, OpenAPI or custom operations.
...
The URL is an existing external MCP service; this package is a client of that service, whereas adapter-mcp publishes your own tools. Configure credentials in the host. `list` returns connector IDs, `describe` requires `connectorId` and returns exact operation IDs/schemas, and `call` requires both IDs plus `arguments`. Discover before calling.

A connector factory creates a new connection per mounted runtime so disposing one run does not close another run's client. Operation listings are cached for 60 seconds and MCP pagination/SSE responses are handled internally. MCP `isError` remains a failure; oversized results carry truncation metadata. OpenAPI adapters preserve base URL path prefixes. Custom `AgentConnector` implementations provide `listOperations`, `call`, and optional health/disposal. The host must enforce action-specific authorization for external writes.
...
```ts
import { createMcpStreamableHttpConnector, createNodeConnectorsCapability } from '@cjfclonedeep/capability-sdk/integrations/connectors/node';
 const url = process.env.BUSINESS_MCP_URL;
 if (!url) throw new Error('Set BUSINESS_MCP_URL to your MCP endpoint');
 const provider = createNodeConnectorsCapability({
   connectors: () => [createMcpStreamableHttpConnector({
     id: 'business', url,
     headers: process.env.BUSINESS_MCP_TOKEN
       ? { authorization: `Bearer ${process.env.BUSINESS_MCP_TOKEN}` } : {},
   })],
 });
...
export const exampleCall = {
  ...
  "input": {
    "action": "list",
    "reason": "Discover available connections"
  }
};
...
Use the self-contained [MCP tutorial](CONNECTORS-MCP.md) shipped with this package. It includes dependency installation, a stdio process, a listening stateful HTTP server, client discovery/calls, a model-driven client Agent, cancellation, authentication boundaries and shutdown. Reuse provider.ts and policy.ts above. A remote client needs only the MCP URL and client dependencies; it does not import this capability.
...
| --- | --- | --- |
| `AGENT_CONNECTOR_TIMEOUT_MS` | `30000` | `runtime` |
| `AGENT_CONNECTOR_MAX_RESULT_CHARS` | `50000` | `runtime` |
...
- `@cjfclonedeep/capability-sdk/integrations/connectors`
- `@cjfclonedeep/capability-sdk/integrations/connectors/node`
- `@cjfclonedeep/capability-sdk/integrations/connectors/mcp`
- `@cjfclonedeep/capability-sdk/integrations/connectors/runtime-skill`
- `@cjfclonedeep/capability-sdk/integrations/connectors/settings`
...
} from '@cjfclonedeep/capability-sdk/integrations/connectors/node';
