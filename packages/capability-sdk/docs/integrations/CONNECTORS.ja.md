...
外部 MCP、OpenAPI、独自の操作を検出して呼び出します。
...
URL は既存の外部 MCP サービスです。本パッケージはそのクライアントであり、adapter-mcp は自分のツールを公開する側です。認証情報はホストに設定します。`list` が接続 ID を返し、`describe` は `connectorId` を受けて操作 ID とスキーマを返します。`call` には両方の ID と `arguments` が必要です。検出してから呼び出してください。

マウントごとにファクトリーで接続を作り、ある実行の解放で別の接続が閉じないようにします。操作一覧は 60 秒キャッシュされ、MCP のページングと SSE は内部で処理されます。MCP の `isError` は失敗として保持され、大きすぎる結果には切り詰め情報が付きます。OpenAPI はベース URL のパス接頭辞を保持します。独自 `AgentConnector` は `listOperations`、`call`、任意の health/dispose を実装します。外部書き込みの操作別承認はホストが担当します。
...
同梱の独立した [MCP 完全ガイド](CONNECTORS-MCP.ja.md) を参照してください。依存導入、stdio プロセス、ステートフル HTTP 待ち受け、クライアントの検出・呼び出し、モデル駆動 Agent、中断、認証境界、終了処理を含みます。上記 provider.ts と policy.ts を再利用します。リモートクライアントは MCP URL とクライアント依存だけでよく、この能力を読み込みません。
