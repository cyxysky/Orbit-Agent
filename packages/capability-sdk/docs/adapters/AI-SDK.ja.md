...
直接接続するには後述の AI SDK 節を使います。mountAISDKCapabilities は host 経由でマウントし、tools、instructions、agentOptions、snapshot、dispose を返します。agentOptions を ToolLoopAgent に展開し、instructions を変更する場合は Skill 内容を保持します。peer 要件は AI SDK 7（ai >=7 <8）です。

既に host でマウントした場合は下位の toAISDKToolSet(snapshot, options) を使い、同じ資源を二重にマウントしません。skills.mode は eager/lazy/disabled です。lazy は明示的な skill ツールを登録し、includeTool: false なら Agent 側が読み取り機構を提供します。Skill の読み取りだけで実行は承認されません。adapter.execute で追跡や操作別承認を行い、提供された invoke() で続行します。adapter.policy は粗い権限、前提条件、進捗を扱います。

アダプターはツール変換を担当し、モデル選択、会話保存、UI、Agent Loop はホストとフレームワークが管理します。/node と /typeorm は host の設定ストアを再公開します。画像はモデルに対応するコンテンツ変換が必要で、パスを含む JSON だけでは画像入力になりません。
...
同梱の独立した [MCP 完全ガイド](AI-SDK-MCP.ja.md) を参照してください。依存導入、stdio プロセス、ステートフル HTTP 待ち受け、クライアントの検出・呼び出し、モデル駆動 Agent、中断、認証境界、終了処理を含みます。上記 provider.ts と policy.ts を再利用します。リモートクライアントは MCP URL とクライアント依存だけでよく、この能力を読み込みません。
