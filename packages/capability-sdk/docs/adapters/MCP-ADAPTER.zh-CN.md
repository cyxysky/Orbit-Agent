...
通过 MCP 发布能力 Provider，独立于使用方 Agent 框架。
...
该适配器是服务端。调用方安装 MCP 客户端即可，不需要安装你的具体能力包。一个服务可暴露多个 Provider，工具与 Skill 名称必须唯一。serveCapabilityMcpStdio 提供进程传输，createCapabilityMcpServer 创建尚未连接的服务对象，createCapabilityMcpHandler 返回 Web Request/Response Handler，不会监听端口。

随包 MCP 教程包含真正监听端口的 HTTP 服务和完整客户端。有状态传输示例为每个初始化的 MCP 会话保留一个能力运行实例；便捷 Handler 使用官方 SDK 的按请求生命周期，内存浏览器管理器不能依赖它跨请求保留状态。

skillMode 默认 eager，lazy 增加显式 skill 工具，disabled 由客户端注入上下文。policy 连接宿主权限、前置条件和进度。产物 URL 转成 MCP resource link；resolveImage(content) 从可信存储解析图片，返回 { data: base64, mimeType }。适配器不托管文件、不鉴权调用方，也不自动访问任意 URL。客户端应将服务指令和完整结构化结果传给 Agent。
...
阅读本包随附、可独立使用的 [MCP 完整教程](MCP-ADAPTER-MCP.zh-CN.md)。包含依赖安装、stdio 进程、有状态 HTTP 监听服务、客户端发现与调用、模型驱动的客户端 Agent、取消、鉴权边界和关闭流程。复用上面的 provider.ts 与 policy.ts。远程客户端只需 MCP URL 及客户端依赖，不导入本能力包。
