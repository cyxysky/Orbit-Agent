# @cjfclonedeep/capability-sdk/mcp

[English](MCP-ADAPTER.md) | [简体中文](MCP-ADAPTER.zh-CN.md) | [日本語](MCP-ADAPTER.ja.md)
...
This adapter is the server side. The caller installs an MCP client and does not install your concrete capability packages. One server can expose multiple providers; tool and Skill names must be unique. `serveCapabilityMcpStdio` serves a process transport; `createCapabilityMcpServer` creates an unconnected server; `createCapabilityMcpHandler` returns a Web Request/Response handler and does not listen on a port.

Follow the included MCP guide for both a listening HTTP server and a complete client. The stateful transport example retains one mounted runtime per initialized MCP session. The convenience handler delegates to the official SDK's per-request lifecycle; an in-memory browser manager must not rely on it for cross-request persistence.

`skillMode` defaults to eager, lazy adds an explicit skill tool, and disabled leaves context injection to the client. `policy` connects host permissions/prerequisites/progress. Artifact URLs become MCP resource links; `resolveImage(content)` must resolve trusted stored image bytes to `{ data: base64, mimeType }`. The adapter does not host files, authenticate callers, or automatically load arbitrary URLs. Client instructions and complete structured tool results must be propagated into the Agent.
...
Use the self-contained [MCP tutorial](MCP-ADAPTER-MCP.md) shipped with this package. It includes dependency installation, a stdio process, a listening stateful HTTP server, client discovery/calls, a model-driven client Agent, cancellation, authentication boundaries and shutdown. Reuse provider.ts and policy.ts above. A remote client needs only the MCP URL and client dependencies; it does not import this capability.
...
`@cjfclonedeep/capability-sdk/host`; see the
...
} from '@cjfclonedeep/capability-sdk/mcp';
