...
Use the AI SDK section below for the direct integration path. `mountAISDKCapabilities` mounts via host and returns `tools`, `instructions`, `agentOptions`, `snapshot`, and `dispose`. Spread agentOptions into ToolLoopAgent; do not replace its instructions without preserving Skill content. AI SDK 7 (`ai >=7 <8`) is the peer requirement.

`toAISDKToolSet(snapshot, options)` is the lower-level path if you already mounted with host; do not mount the same resources twice. `skills.mode` is eager/lazy/disabled. Lazy mode registers an explicit `skill` tool; `includeTool: false` means your Agent must supply its own reader. Reading a Skill does not automatically authorize execution. `adapter.execute` may wrap tracing or action approvals; call its provided `invoke()` to continue. `adapter.policy` provides coarse permission checks, prerequisites and progress.

The adapter owns tool conversion, not model selection, conversation persistence, UI, or the Agent Loop. `/ai-sdk/node` and `/ai-sdk/typeorm` re-export host configuration stores. Images need a model-specific result/content mapping; returning JSON with an image path is not visual input.
...
Use the self-contained [MCP tutorial](AI-SDK-MCP.md) shipped with this package. It includes dependency installation, a stdio process, a listening stateful HTTP server, client discovery/calls, a model-driven client Agent, cancellation, authentication boundaries and shutdown. Reuse provider.ts and policy.ts above. A remote client needs only the MCP URL and client dependencies; it does not import this capability.
...
through `@cjfclonedeep/capability-sdk/host`. See the
[framework-neutral integration guide](../../FRAMEWORK_INTEGRATION.md).
...
} from '@cjfclonedeep/capability-sdk/ai-sdk/node';
...
The adapter depends on `@cjfclonedeep/capability-sdk/host`, so consumers do not install
the host package separately unless they want to import its framework-neutral
APIs directly. The `/ai-sdk/node` and `/ai-sdk/typeorm` adapter entrypoints re-export the
corresponding built-in configuration stores. Using `/ai-sdk/typeorm` requires the host
