# @cjfclonedeep/capability-sdk/knowledge
...
Persist reference documents and search their indexed text.
...
After `ingest`, retain the returned document ID. Use `search` with `query`, `get` with `documentId`, `list` with pagination, and `delete` with the exact ID. Search results include document/chunk provenance; preserve it in answers. The Node store is SQLite FTS5 with Chinese bigram tokenization, not an embedding service.

Supply extracted text in `content`; use File or your own ingestion pipeline to read PDF/Office uploads first. Keep a stable, host-selected directory for durable retrieval and isolate tenants by directory. Back up the SQLite database consistently with its WAL; dispose before deleting the store. A custom `KnowledgeStore` can provide vector/hybrid search through `createKnowledgeCapability`.
...
import { createNodeKnowledgeCapability } from '@cjfclonedeep/capability-sdk/knowledge/node';
 const provider = createNodeKnowledgeCapability({ directory: './agent-data/knowledge' });
...
export const exampleCall = {
  ...
  "input": {
    "action": "ingest",
    "reason": "Save reference material",
    "title": "Office hours",
    "content": "Support is available Monday through Friday, 09:00–18:00."
  }
};
...
| --- | --- | --- |
| `AGENT_KNOWLEDGE_CHUNK_CHARS` | `1800` | `runtime` |
| `AGENT_KNOWLEDGE_SEARCH_LIMIT` | `8` | `runtime` |
...
- `@cjfclonedeep/capability-sdk/knowledge`
- `@cjfclonedeep/capability-sdk/knowledge/node`
- `@cjfclonedeep/capability-sdk/knowledge/mcp`
- `@cjfclonedeep/capability-sdk/knowledge/runtime-skill`
- `@cjfclonedeep/capability-sdk/knowledge/settings`
