...
通过可替换执行器运行受限 JavaScript/Python 计算。
...
已有隔离 HTTP runner 时，从 `/code/remote` 使用 `createHttpCodeSandboxCapability({ url, token })`，runner 由使用方独立部署。仅设置 `AGENT_CODE_SANDBOX_BACKEND` 不会改变显式选择的工厂函数。本地工厂无法实现 `networkMode: "none"`，CPU、内存和网络隔离需要执行后端提供。

最小工厂不发布输出文件。如果代码生成 outputs 文件，应通过 `createCodeSandboxCapability({ createExecutor })` 包装 `createNodeProcessCodeSandbox` 或 `createHttpCodeSandboxExecutor`：持久化内部 `files[].base64`，以 artifacts 替换 files，实现 readFile，并把 artifactInputs 解析为传输 inputFiles。不要把内部 Base64 交给模型。下面提供具体的产物适配示例。释放运行实例会删除本地执行工作区，发布的文件应保存到其他目录。stdout/stderr 截断与进程失败是不同状态。
...
阅读本包随附、可独立使用的 [MCP 完整教程](CODE-MCP.zh-CN.md)。包含依赖安装、stdio 进程、有状态 HTTP 监听服务、客户端发现与调用、模型驱动的客户端 Agent、取消、鉴权边界和关闭流程。复用上面的 provider.ts 与 policy.ts。远程客户端只需 MCP URL 及客户端依赖，不导入本能力包。
...
可选替换 provider.ts 中的能力创建代码，保留 configurations/exampleCall/cleanup 导出，合并时去掉重复 provider 导出。本包装器实现每轮产物注册、输出发布、有界分页读取及后续输入挂载。跨服务重启需要把注册元数据保存到你的存储，并实现下载路由。worker 仅持有临时目录。HTTP runner 可将 worker 工厂替换为 /remote 的 createHttpCodeSandboxExecutor({url,token})。输出通道限制为 16 个文件、每个 10 MB、合计 32 MB；runner 拒绝符号链接与越界路径。

已有仓库说明中的生产 runner 和 Dockerfile 属于 Orbit 应用，并不随本 npm 包发布。独立宿主需要提供实现 POST /execute、GET /health 的隔离 runner（供 /remote 调用），或自有 CodeSandboxExecutor。本地示例明确用于可信单机场景。
