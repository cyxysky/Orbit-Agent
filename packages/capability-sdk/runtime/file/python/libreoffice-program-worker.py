...
# Runtime asset distributed with @cjfclonedeep/capability-sdk/file.
import argparse
import ast
import base64
import builtins
...
def postprocess_ooxml(job):
    ...
    # Validation output names already contain a document id and source digest.
    # Repeating that basename plus a UUID exceeds Windows MAX_PATH even when
    # the output itself is valid. Keep the sibling short for atomic replacement.
    temporary = job.output_path.with_name(f'wp-ooxml-{uuid.uuid4().hex}.tmp')
    try:
