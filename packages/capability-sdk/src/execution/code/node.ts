...
import { collectFiles, stageFiles } from '../../../runtime/execution/files.cjs';
  ...
  return {
    async run(execution, context): Promise<CodeSandboxExecutionResult> {
      ...
      try {
        ...
        const executable = execution.language === 'python'
          ? executableName(input.pythonExecutable, readManagedRuntime()?.python || (process.platform === 'win32' ? 'python' : 'python3'))
    ...
    async health() {
      try {
        ...
        const pythonError = await checkExecutable(executableName(input.pythonExecutable, readManagedRuntime()?.python || (process.platform === 'win32' ? 'python' : 'python3')));
