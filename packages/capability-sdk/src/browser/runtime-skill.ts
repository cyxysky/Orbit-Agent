...
- \`browser({ action: "dismissSurface", reason })\` always sends one direct \`page.mouse.click(0, 0)\`, without Locator actionability checks, Escape, or a required surface id. The tool returns the before/after surface observations, \`data.closureConfirmed\`, \`data.outcome\`, a bounded \`data.postActionState\`, and a final screenshot when available. Outer \`ok: true\` means the click was delivered; it does not mean a popup closed. If no active surface was detected beforehand, the click still runs and closure is unconfirmed. Inspect the returned state before acting behind the popup. This fixed corner click is for popup dismissal only, not general navigation or control selection.
...
The outer browser action=code result is \`{ ok, summary, data, failureCategory?, referenceImagePaths?, browserObservation? }\`. \`data\` contains \`{ result, finalPage?, observation?, error?, aborted?, executionState?, downloads?, kernelReset?, postActionState?, images?, imageErrors? }\`. Read it directly without JSON parsing. When BROWSER_CODE_AUTO_SCREENSHOT is enabled (the default), every code call captures the final active viewport after the ENTIRE cell, including after failed scripts when the page remains accessible. When image input and capture are available, the next model request attaches the actual latest viewport pixels in a \`[Current browser observation]\` image block after the tool receipt. Inspect that image together with \`data.result\`, \`data.error\`, \`executionState\` and any explicit DOM read before a dependent action; a file path, observation ID, tool reason or outer \`ok\` is not the image or a verified outcome. No separate image-read call is needed for attached pixels. When disabled, data.observation.status is disabled, no automatic image is supplied, and explicitly emitted reference images still work. \`observation\` reports capture status or error; full screenshot metadata remains in the raw trace. Only the latest automatic browser observation remains in the model request; older screenshots remain execution artifacts, not current state. If capture fails or the model cannot accept images, explicitly read live DOM/Playwright state and do not claim visual inspection. No DOM delta is returned. Use \`page.domSnapshot()\` or targeted reads for exact locator attributes. Screenshots show visible state, not hidden attributes or proof of business completion. Wait for the relevant state and verify the outcome; split visually dependent UI transitions into separate cells or read the changed DOM before continuing inside a cell. A declared browser action with zero attempted actions was not executed, even if the cell returned a value; Browser Chat returns \`browser-no-action\` and a bounded live \`data.recoveryState\` to diagnose it. Failed results preserve the complete error and failure classification.
...
5. Do not assume selection auto-closes the popup. If it remains open, use its observed Apply/Done/OK/Close control or the observed trigger. For a popup that should close on an outside click, use browser action=dismissSurface, which clicks viewport (0,0).
6. Verify the field value and that this exact surface closed before targeting outside it. A remaining parent dialog is expected. Combine a known selection and its explicit close control in one cell when possible; otherwise inspect the separate dismissSurface result before continuing. Do not click the trigger again after the popup has closed, as that reopens it.

Prefer an already observed explicit close/confirm control, or the original trigger when evidence shows it toggles this popup. Use dismissSurface for an outside-click dismissal; it always sends the click even when surface detection misses the popup, and reports whether closure was confirmed. Record a no-op dismissal and do not repeat it unchanged. After two distinct attempts with no change, return one consolidated targeted diagnosis (surface id/stack, trigger, visible controls and blocker), then choose a new evidence-supported action. Never remove overlay DOM, change styles, or bypass the application's close handlers. Remember a verified dismissal method for the same control during this task, and re-check the live popup before reusing it.
