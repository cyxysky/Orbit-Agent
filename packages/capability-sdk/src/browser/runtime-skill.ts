...
2. Call \`browser({ action: "code", reason, code })\` directly for every new or resumed browser request. There is no automatic DOM preflight. When BROWSER_CODE_AUTO_SCREENSHOT is enabled (the default), a final viewport screenshot is captured after each code call. For a known URL, navigate and read the new page in the same cell without first reading the page being left. Call \`browser({ action: "state", reason })\` explicitly only when its snapshot is itself the desired result.
...
The outer browser action=code result is \`{ ok, summary, data, failureCategory?, referenceImagePaths?, browserObservation? }\`. \`data\` contains \`{ result, finalPage, observation, error?, aborted?, executionState?, downloads?, kernelReset?, verification?, images?, imageErrors? }\`. Read it directly without JSON parsing. When BROWSER_CODE_AUTO_SCREENSHOT is enabled (the default), every code call captures the final active viewport, including after failed scripts when the page remains accessible. When disabled, observation.status is disabled, no automatic image is supplied, and explicitly emitted reference images still work. \`observation\` reports capture status, URL and time, or the capture error. Image-capable hosts provide only the latest browser observation to the next model request; older screenshots remain execution artifacts, not current state. If capture fails or the model cannot accept images, explicitly read live DOM/Playwright state. No DOM delta is returned. Use \`page.domSnapshot()\` or targeted reads for exact locator attributes. Screenshots show the captured visible state, not proof that a business operation finished. Wait for the relevant state and verify the outcome; split dependent UI transitions into separate cells or read the changed DOM before continuing inside a cell. Failed results preserve the complete error and failure classification.
