...
5. Every code cell that performs a browser action must call \`page.verifyState()\` after its last action, using a concrete URL, locator value/state, or exact surface transition. An unverified action returns \`browser-verification-required\` with a fresh state read even when Playwright completed it; do not replay that action. If an overlay opened or a picker remained open, the assertion must explicitly check its surface state. An acknowledged still-open picker is an intermediate state, not completion: close or confirm it before acting outside it. A separate verification tool call is unnecessary when the assertion can follow the action in the same cell. For read-only research, returned relevant page content is the evidence.
...
- \`browser({ action: "dismissSurface", reason, surfaceId, dismissMethod? })\` dismisses only the exact currently active surface observed by id, then verifies that it disappeared. Default method is Escape; backdrop clicks are sent only at a live hit-tested backdrop point outside the surface. A failed/no-op result includes fresh state. Never blindly force-click viewport coordinates such as (0,0).
...
- \`await page.verifyState(input): Promise<{ ok: true, description, checks, observation }>\` is required after the last browser action in a code cell. \`input.description\` is required. Supply at least one of \`url\`, \`activeSurface\`, or \`locator\` evidence. A passing assertion made before a later action does not verify that later action.
...
An overlay root may be a full-viewport backdrop while its visible dialog or dropdown is another surface. DOM order is not visual stacking: do not scope a Cancel/Close button through \`page.locator('[id*=overlay]').last()\` merely because it is the last overlay node. A failed browser result includes live \`diagnostics.surfaces\`, \`surfaceStack\`, and visible \`controls\` with any center-point \`blocker\`; use them before changing the script. When a backdrop intercepts a visible button, dismiss the foreground popup first. Put \`await page.keyboard.press('Escape')\` before any locator that previously failed, then read the surface/visible options; if Escape did not dismiss it, click the observed backdrop or another observed control and verify. Once the blocker is gone, resolve the dialog's actual Cancel/Close button by its current accessible name or observed surface selector and verify the dialog closed. A failed first statement prevents later Escape or Cancel statements in the same cell from running; do not hide the recovery behind a failed click. \`force: true\`, raw coordinates, and SPA navigation are not dismissal strategies for a covered element.
