...
import { readBoundedResponseText } from '../index.ts';
...
function windowsDriverScriptPath() {
  ...
  const candidates = [
    ...
    path.join(process.cwd(), 'packages', 'capability-sdk', 'runtime', 'computer', fileName),
    fileURLToPath(new URL(`../../runtime/computer/${fileName}`, import.meta.url)),
  ].filter(Boolean);
...
async function sharedBuiltInWindowsDriver() {
  ...
  const pending = builtInDriverService;
  builtInDriverUsers += 1;
  let service: BuiltInDriverService;
  try { service = await pending; }
  catch (error) { builtInDriverUsers -= 1; throw error; }
  const driver = createRemoteComputerDriver({
    endpoint: service.endpoint,
    headers: { authorization: service.authorization },
  });
  let disposed = false;
  return { ...driver, async dispose() {
    if (disposed) return;
    disposed = true;
    builtInDriverUsers -= 1;
    if (builtInDriverUsers === 0 && builtInDriverService === pending) {
      builtInDriverService = undefined;
      await service.close();
    }
  } };
