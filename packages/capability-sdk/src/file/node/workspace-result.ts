...
import type { createFileVisualIndex } from './visual-index.ts';
...
export type ArtifactToolPayload = {
  ...
  visualIndex?: ReturnType<typeof createFileVisualIndex>;
};
...
export function formatFileArtifactResult(toolName: string, actual?: string) {
  ...
  try {
    ...
    // The model uses this formatter too. Preserve the render-to-read protocol
    // instead of reducing the screenshot index to a page-count summary.
    if (payload.visualIndex) {
      return JSON.stringify({
        kind: payload.kind, documentId: payload.documentId,
        artifactId: payload.artifactId, fileName: payload.fileName,
        downloadUrl: payload.downloadUrl, url: payload.url, bytes: payload.bytes,
        visualIndex: payload.visualIndex,
        nextAction: payload.visualIndex.nextRead,
        workflow: payload.workflow,
        automaticValidation: compactAutomaticValidation(payload),
        visualVerification: compactVisualVerification(payload),
      });
    }
