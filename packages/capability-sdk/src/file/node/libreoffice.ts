...
export async function resolveLibreOfficeExecutable(
  ...
  const candidates = [
    configured,
    readManagedRuntime(environment)?.libreoffice || '',
  ...
  const probe = new Promise<boolean>((resolve) => {
    ...
    execFile(executable, ['-c', 'import uno'], {
      ...
      env: {
        ...
        ...(process.platform === 'linux' ? { LD_LIBRARY_PATH: [libreOfficeProgramDirectory, path.resolve(path.dirname(executable), '../lib'), environment.LD_LIBRARY_PATH].filter(Boolean).join(path.delimiter) } : {}),
...
export async function resolveLibreOfficePythonExecutable(
  ...
  const candidates = [
    configured,
    readManagedRuntime(environment)?.unoPython || '',
