...
export async function resolveUnoProgramWorker(options: {
  ...
  const bundled = path.resolve(moduleDirectory, '..', '..', '..', '..', 'runtime', 'file', 'python', 'libreoffice-program-worker.py');
  const candidates = [
    configured,
    ...
    path.join(process.cwd(), 'packages', 'capability-sdk', 'runtime', 'file', 'python', 'libreoffice-program-worker.py'),
    path.join(process.cwd(), 'node_modules', '@cjfclonedeep', 'capability-sdk', 'runtime', 'file', 'python', 'libreoffice-program-worker.py'),
  ].filter(Boolean);
  ...
  return new Promise<Record<string, unknown>>((resolve, reject) => {
    ...
    const child = spawn(input.executable, input.args, {
      ...
      env: {
        ...
        ...(process.platform === 'linux' ? { LD_LIBRARY_PATH: [input.libreOfficeProgramDirectory, path.resolve(path.dirname(input.executable), '../lib'), process.env.LD_LIBRARY_PATH].filter(Boolean).join(path.delimiter) } : {}),
