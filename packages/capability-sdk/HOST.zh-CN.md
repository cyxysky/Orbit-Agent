...
挂载 Provider，统一配置、工具选择和可移植 Skill 目录。
...
Greeting 示例不需要外部服务，可替换为任意具体能力工厂。mountCapabilities 注册 Provider、读取配置、补默认值并创建工具/Skill 快照，不创建 Agent，也不调用模型。只有你的代码直接使用 SDK 类型或执行器时才需声明 SDK 为直接依赖。

配置优先级为按能力显式覆盖 > 存储值 > context.configuration，缺失或无效项再使用包默认值。configScope 选择用户/工作区/profile 的存储范围，不是鉴权机制。enabledCapabilityIds 和 allowedToolNames 筛选注册内容。保存配置不会改变已经挂载的快照；runtime 设置变化后重新挂载，startup 设置还需重新创建所属服务/驱动。

存储入口：根入口提供 Memory、Environment，/node 提供 JsonFile，/typeorm 提供 TypeOrm。TypeORM 需要宿主初始化 DataSource，将 typeOrmCapabilityConfigurationEntity 加入 entities，并通过迁移或原型建表创建结构。createSplitCapabilityConfigStore({ values, secrets }) 分离机密存储。设置定义归能力包，存储选择归宿主。Skill 目录返回 eager 全文或 lazy 摘要；只有摘要并不会自动创建 Skill 工具。
...
阅读本包随附、可独立使用的 [MCP 完整教程](HOST-MCP.zh-CN.md)。包含依赖安装、stdio 进程、有状态 HTTP 监听服务、客户端发现与调用、模型驱动的客户端 Agent、取消、鉴权边界和关闭流程。复用上面的 provider.ts 与 policy.ts。远程客户端只需 MCP URL 及客户端依赖，不导入本能力包。
