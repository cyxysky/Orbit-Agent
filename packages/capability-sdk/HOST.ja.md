...
正規化された設定、ツール選択、移植可能な Skill カタログとともに Provider をマウントします。
...
Greeting 例は外部サービス不要で、任意の能力ファクトリーに置き換えられます。mountCapabilities は Provider を登録し、設定を読み、既定値を適用してツール/Skill スナップショットを作ります。Agent やモデル呼び出しは作りません。自分のコードで SDK の型や実行器を直接使う場合に SDK を直接依存として宣言します。

設定の優先順位は能力別の明示上書き > 保存値 > context.configuration で、不足・無効値には既定値を適用します。configScope はユーザー/ワークスペース/profile の保存範囲であり認証ではありません。enabledCapabilityIds と allowedToolNames は登録を絞ります。保存しても既存スナップショットは変わりません。runtime 設定は再マウント、startup 設定は所有サービス/ドライバーの再作成も必要です。

ストアはルートの Memory/Environment、/node の JsonFile、/typeorm の TypeOrm があります。TypeORM はホストが初期化した DataSource と entities 内の typeOrmCapabilityConfigurationEntity、およびマイグレーション等によるテーブル作成が必要です。createSplitCapabilityConfigStore({ values, secrets }) で秘密情報を分離します。設定定義は能力パッケージ、保存先はホストが所有します。Skill カタログは eager の全文か lazy の要約を返しますが、要約だけでは Skill ツールは作られません。
...
同梱の独立した [MCP 完全ガイド](HOST-MCP.ja.md) を参照してください。依存導入、stdio プロセス、ステートフル HTTP 待ち受け、クライアントの検出・呼び出し、モデル駆動 Agent、中断、認証境界、終了処理を含みます。上記 provider.ts と policy.ts を再利用します。リモートクライアントは MCP URL とクライアント依存だけでよく、この能力を読み込みません。
