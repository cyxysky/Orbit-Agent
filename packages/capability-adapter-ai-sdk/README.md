...
The adapter owns tool conversion, not model selection, conversation persistence, UI, or the Agent Loop. `/node` and `/typeorm` re-export host configuration stores. Images need a model-specific result/content mapping; returning JSON with an image path is not visual input.
