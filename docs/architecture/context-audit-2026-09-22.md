...
| Skill 可见性、用户原文与资料状态 | `hidden-runtime-skills.ts`、`runtime-knowledge-context.ts`、`runtime-context-runtime.ts` |
...
- 用户原文：直接保留原始用户消息及后续纠正，不经任务笔记登记；`taskContext` 工具及运行时状态注入已移除。
...
这些检查验证代码机制与真实记录的恢复，不证明真实模型摘要的语义保真，也不替代真实供应商和浏览器端到端验证。Token 仍是启发式估算，供应商 tokenizer 和媒体计费可能不同；供应商实际拒绝时走有进展的压缩重试。摘要是有损历史，用户原文及已加载 Skill 正文直接保留，历史证据通过 contextRead 取回。若必需原文和必须保留的近期交互本身超过模型容量，系统会明确停止，而非伪造压缩成功。

## 补充：移除任务笔记工具并保护原始指令

- 移除 taskContext 工具目录、原生注册、Codex 分发、类型与服务、状态注入及对应建表迁移；不再要求模型登记、pin 或 transition。没有操作正在使用的数据库或清理其历史数据。
- 用户 Skill 原先虽然 required=true，却同时 bodyAvailable=false/resourceOnly=true，实际被组装器跳过；现按有效权限和版本持续注入完整正文，附件资源保持按需读取。
- 系统 Skill 的最近一次完整读取交互、原始用户消息不进入有损压缩；跳过保护块后继续处理后续普通历史。重复 Skill 读取只保护最近一份完整正文，不拆分工具调用与结果。
- Skill 指导明确区分页面事实和用户约束：实际页面可以推翻过期 UI 描述，不能据此跳过用户要求或必需操作。后续用户纠正优先于早期冲突要求，摘要不得覆盖原文。
- 内联检查验证了重复压缩、完整工具协议、原文顺序、必需原文超容量停止、用户 Skill 正文注入、版本更新与停用。原文在场改善遵循所需的信息条件，不保证模型一定正确理解；未做真实供应商的语义遵循验证。

## 补充：DOM 截图交付与文件原文保真

DOM 模式原先在执行入口禁用截图、在模型准备时跳过截图、在最终投影中移除图片，Skill 还明确禁止看图。现将操作方式与结果证据分开：DOM 仍使用定位器操作，支持图片时附加最新视口。模型直接接收像素，不需要再调用读图工具；提示词要求在下一次依赖操作前综合 DOM、浮层、输入值、校验提示和加载状态。新采集失败会淘汰旧自动截图；图片能力缺失或自动截图关闭会明确通知。manifest 和日志统计最终请求 browserScreenshotCount，不将“已附加”记成“模型已理解”。

文件分页结果此前再次经过约 1200 tokens 的通用工具预览，随后又进入长消息预览和有损 handoff，尤其容易丢失正文中间内容。现文件原始读取范围独立归档、索引和注入，普通执行历史只保留来源回执。沿活动分支及 handoff 来源恢复，保护范围覆盖连续多次压缩；检查点同步保存新引用回执，避免第二次恢复时断链。资料超过输入容量的 45% 时检索精确原文片段，同时提供所有已读范围的定位信息和实际附加范围；这不是整份超大文件无损塞入模型，遗漏细节需要原文回读。损坏或缺失的原文引用显式给出原文件重读入口，不能把“读取完成”提示当文件正文。

只读核对 chat_24a36d123e4b 的真实 SQLite 工具记录：5 次成功文件读取去重为 4 个范围，56,754 个原文字符全部进入资料上下文。内联检查覆盖多次压缩后原文中间细节、精确回读偏移、大文件片段检索、分支范围隔离、重复读取去重和图片投影。隔离 Chromium 验证了 DOM 点击 → SDK 操作后 PNG → 最终模型消息包含相同 PNG 字节。该检查没有调用真实模型，不证明模型已经正确理解截图或执行完整业务验收。未创建测试文件、未运行 dev/build。
