...
建议新建一个专用测试会话，全程打开请求日志。重点核对 **模型收到什么、实际做什么、上下文统计是否一致**。

| 测试项 | 操作与测试内容 | 预期结果／目的 |
|---|---|---|
| **1. 基础对话** | 发送：“记住本次测试信息：项目叫青禾，预算8000元，交付日期10月20日。只回复收到。”再问：“项目名称、预算和日期是什么？” | 正确回答三个信息，验证基础历史传递。 |
| **2. 重复消息** | 连续两次发送“1+1”。 | 两次都正常回复；请求中保留两次真实提问，不因文字相同而去重。 |
| **3. 切换任务** | 先让模型浏览网页并整理信息，再发送：“现在只回答1+1，不继续之前的任务。” | 回答2，不继续浏览；当前问题之后不出现自动追加的旧任务待办或材料清单。 |
| **4. 中止后换任务** | 发起一个需要多次工具调用的任务，等部分工具完成后中止，再发送“2+2，只回答结果”。 | 回答4；已完成的工具记录仍保留，没有伪造的“中止回复”干扰模型。 |
| **5. 中止后继续** | 另起一次多步骤任务，中途停止，再明确要求：“继续刚才的任务，先说明已完成和未完成的部分。” | 能利用已保存的信息继续，不把已经完成的步骤全部重新执行。 |
| **6. 工具调用与大结果** | 让模型读取一份较长文档或网页，再追问其中一个具体细节。 | 调用与结果配对；同一结果不会重复注入。大型结果可显示有限预览和引用，必要时能回读原文。 |
| **7. 自动压缩** | 连续发送较长材料，使上下文达到触发阈值，再发送一个新问题。 | 出现明确的压缩进度；压缩成功后继续回答，不长期停在普通“AI请求中”，也不直接发送超限输入。 |
| **8. 压缩保真** | 压缩后追问测试1的项目、预算和日期，以及长材料中的关键事实。 | 保留关键事实；不确定的细节应回读原文，不能编造。 |
| **9. 信息修正** | 压缩前发送：“预算改为6500元，日期改为10月25日，其他不变。”压缩后再问当前预算和日期。 | 回答更新后的值，不让旧摘要覆盖后来的更正。 |
| **10. 压缩后切换任务** | 确认压缩完成后发送：“1+1，只回答结果。” | 回答2；摘要中的旧目标不能成为新的执行命令。 |
| **11. 多次压缩** | 继续积累材料，至少再触发一次压缩，然后追问早期事实和最新修改。 | 摘要持续更新，不不断堆叠；早期重要信息和最新修正仍可恢复。 |
| **12. 压缩中取消** | 压缩过程中点击停止，再刷新页面并发送新问题。 | 停止等待；已成功保存的压缩批次保留，未成功的部分不丢失，可以继续对话。 |
| **13. 刷新恢复** | 分别在普通回复后、中止后、压缩后刷新页面，再继续提问。 | 历史、摘要和上下文统计能够恢复，不突然丢失中止内容或恢复旧任务状态。 |
| **14. 模型切换** | 在有较长历史的会话里切换到窗口更小的模型，再发送问题。 | 使用新模型的预算，必要时先压缩；界面窗口上限与当前模型配置一致。 |
| **15. Skill与记忆** | 在任务中明确选择相关Skill，下一轮切换到不相关的问题。 | 必要规则正常加载；旧任务读取的大量资料不会无条件、重复进入每轮请求。 |

**触发压缩的办法**

可以临时降低压缩触发比例，避免为了测试消耗大量 token。记录原值，测试后恢复；目标比例要低于触发比例。长材料最好带有可核对的编号、事实和后续修正，不能只用重复字符——否则无法判断压缩是否保留了有用信息。

**每次重点检查请求日志**

- 展示实际消息和工具定义，没有 `contextManifest`、`modelContextSegmentation` 等内部诊断清单。
- 请求顺序合理：必要背景在前，真实对话按时间排列；首次响应当前问题时，最新用户消息没有被生成的旧任务内容挤到前面。
- 压缩后的请求包含一份历史摘要、保留的近期交互和当前问题；不会再次夹带全部旧原文。
- 压缩前后 token 变化合理，中止后不会无故大幅减少，窗口上限不会自行变化。
- 后续工具执行时，追加真实工具调用和结果属于正常行为。
...
**你提出的“Next 前端＋独立 Node 后端”，是更合适的长期方向。** 当前只是进程分开，API 进程内部仍运行 Next，所以还要处理框架间接加载、404 渲染等入口。去掉后端的 Next，可以直接消除这一类依赖问题。

不过，前后端分离不能保证所有 OOM、接口阻塞都消失：如果 Agent 的同步计算、序列化或堆快照仍与 API 共用事件循环，中止接口仍可能被拖住。需要把这些耗时工作进一步放到 worker 或子进程。

这个项目**业务重写量较小，但运行和打包边界需要认真调整**。刚统计到：

- **71 个 API 路由文件、105 个处理函数**，多数已经调用独立的 `src/server` 服务。
- **14 个路由文件**使用 `request.nextUrl`，可以改为标准 `URL`。
- 设置页、管理页等少量页面还直接读取后端服务，需要改为 API 读取。
- SSE、WebSocket、文件上传下载、身份校验、后台任务启动，以及三种打包脚本，都需要接到新的 Node 入口。
- 后端 TypeScript、路径别名和 SDK 编译目前依赖 Next，需要独立构建。

我建议保持现有公开端口和 API 地址，内部改为 **Next 负责页面，Node 负责 API 与业务，耗时 Agent 工作由独立执行进程承载**。这样前端调用和大部分业务逻辑可以保留，也能删除现在这套开发目录镜像、React 别名和 Next 路由规避代码。
