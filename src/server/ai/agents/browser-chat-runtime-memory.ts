...
export function compactBrowserChatLogDetails(details: string | undefined, maxCharacters: number) {
  ...
  try {
    ...
    // Request bodies are evidence, not previews. Count/age eviction bounds the
    // in-memory log list; the durable log and UI retain the actual model input.
    if (parsed?.aiInput !== undefined) return details;
