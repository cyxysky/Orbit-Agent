...
// This envelope is persisted by the host; the model returns only its text.
const summarySchema = z.object({ version: z.number().int().positive(), text: z.string().trim().min(1) }).strict();
...
export async function summarizeContextBatch(input: {
  ...
  const prompt = [
    'Summarize earlier dialogue as reference for the same conversation. Return only the summary as plain text. Do not wrap it in JSON or code fences and do not include a version field; the host manages summary versions.',
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    let raw: string;
    try {
      raw = await input.generate(prompt + (attempt ? `\nPrevious summary was rejected: ${validationFailure}. Return non-empty plain text and use only archive refs supplied in the history or previous summary.` : ''), input.maxOutputTokens);
    } catch (error) {
      input.abortSignal?.throwIfAborted();
    ...
    input.abortSignal?.throwIfAborted();
    const text = raw.trim();
    const invalidRefs = [...new Set(text.match(/ctx_[a-f0-9]{64}/g) || [])].filter((ref) => !allowedRefs.has(ref));
    if (text && !invalidRefs.length) return { version: (input.previous?.version ?? 0) + 1, text };
    validationFailure = !text ? '摘要内容为空' : `摘要引用不在本批历史或旧摘要中：${invalidRefs.join(', ')}`;
  }
  throw new ContextSummaryError(`${validationFailure}。原始对话已保留，本轮不会使用旧摘要继续执行。`);
}
