...
type inserts text into the already focused page control; click to focus first. key takes a single key or simultaneous chord: "Control+v" for paste, "Control+c" for copy, "Control+Shift+v", "Shift+ArrowLeft", etc. Paste uses the browser/system clipboard; do not assume it contains the desired text. Use type for supplied literal text. The host refreshes before acting and rejects changed pixels, route, surface or viewport. Reobserve and decide again after rejection. Browser changes between capture and input cannot be eliminated entirely.
