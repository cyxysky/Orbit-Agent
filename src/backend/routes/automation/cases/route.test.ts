...
test('automation case history defaults to stable pages of ten', async () => {
  ...
  try {
    ...
    const firstResponse = await GET(new Request('http://localhost/api/automation/cases', { headers }));
    ...
    const secondResponse = await GET(new Request(`http://localhost/api/automation/cases?${params}`, { headers }));
