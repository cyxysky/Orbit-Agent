...
export async function GET(request: Request) {
  ...
  const records = await listAutomationCases({
    ...
    sourceSessionId: new URL(request.url).searchParams.get('sourceSessionId')?.trim() || undefined,
    beforeId: new URL(request.url).searchParams.get('beforeId')?.trim() || undefined,
    beforeUpdatedAt: new URL(request.url).searchParams.get('beforeUpdatedAt')?.trim() || undefined,
    limit: limit + 1,
  });
