...
export async function GET(request: Request) {
  ...
  const sessionLimit = boundedQueryInteger(new URL(request.url).searchParams.get('sessionLimit'), { fallback: 10, max: 100 });
  const skillLimit = boundedQueryInteger(new URL(request.url).searchParams.get('skillLimit'), { fallback: 50, max: 100 });
