...
test('skills API provides stable cursor pagination and server-side search', async () => {
  ...
  try {
    ...
    const firstResponse = await GET(new Request('http://localhost/api/skills?limit=3', { headers }));
    ...
    const secondResponse = await GET(new Request(`http://localhost/api/skills?${cursor}`, { headers }));
    ...
    const searchResponse = await GET(new Request('http://localhost/api/skills?limit=10&q=unique%20server%20query', { headers }));
