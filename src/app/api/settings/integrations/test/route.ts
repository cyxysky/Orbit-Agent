...
import {
  ...
  type ExternalIntegrationTestTarget,
...
const testSchema = z.object({
  ...
  selectedTarget: z.object({ kind: z.enum(['user', 'group']), id: z.string().trim().min(1).max(500) }).strict().optional(),
}).strict();
...
function streamIntegrationTest(request: NextRequest, candidate: ResolvedExternalIntegration, timeout: number, selectedTarget?: Pick<ExternalIntegrationTestTarget, 'kind' | 'id'>) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (request.nextUrl.searchParams.get('stream') === '1') {
      return streamIntegrationTest(request, candidate, timeout, body.selectedTarget);
    }
    return apiJson(request, { ok: true, result: await testExternalIntegration(candidate, timeout, request.signal, undefined, body.selectedTarget) });
  } catch (error) {
