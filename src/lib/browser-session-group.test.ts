...
import { browserSessionGroupLabel } from '@cjfclonedeep/capability-sdk/browser';
...
test('the Chrome tab-group extension uses the same canonical label', () => {
  const source = readFileSync(path.join(
    process.cwd(),
    'packages/capability-sdk/runtime/browser/session-tab-grouper-extension/group-title.js',
  ), 'utf8');
