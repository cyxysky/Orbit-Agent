...
const applicationRuntimeEnvDefinitions: RuntimeEnvDefinition[] = [
  ...
  { key: 'AI_CONTEXT_COMPRESSION_TRIGGER_RATIO', label: '上下文压缩触发比例', description: '相对于当前模型最大上下文的比例，范围 0.01–0.99；0.85 表示达到 85% 时触发压缩。单次输出上限不参与计算。', tab: 'runtime', defaultValue: '0.85', control: 'number', min: 0.01, max: 0.99, step: 0.01 },
  { key: 'AI_CONTEXT_COMPRESSION_TARGET_RATIO', label: '压缩后建议比例', description: '相对于模型上下文窗口的目标比例，范围 0.01–0.99，目标始终低于触发阈值。优先保留当前请求与最新完整交互，实际结果可能高于目标；实际请求限制由模型服务商校验。', tab: 'runtime', defaultValue: '0.25', control: 'number', min: 0.01, max: 0.99, step: 0.01 },
